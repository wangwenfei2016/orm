...
public class userManageController extends BaseController {
	...
	public String user(HttpServletRequest request) {
		...
		departList.clear();
		return "com/xymn/userManage/userList";
	}
	...
	public String userinfo(HttpServletRequest request) {
		...
		List<TSUser> newusers=userService.findByProperty(TSUser.class,"id",currentUser.getId());
		if(newusers.size()>0){
			request.setAttribute("user", newusers.get(0));
			idandname(request, newusers.get(0));
			getOrgInfos(request, newusers.get(0));
		}
		return "com/xymn/userManage/userinfo";
	}
	...
	public ModelAndView changepasswordforuser(TSUser user, HttpServletRequest req) {
		...
		return new ModelAndView("com/xymn/userManage/adminchangepwd");
	}
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        if(StringUtil.isNotEmpty(request.getParameter("tableStatus"))){
			cq.eq("tableStatus", Integer.valueOf(request.getParameter("tableStatus")));
		}
	...
	public String deleteDialog(TSUser user,HttpServletRequest request) {
		request.setAttribute("user", user);
		return "com/xymn/userManage/user-delete";
	}
	...
	public String toUpdateTableStatus(TSUser user,HttpServletRequest request) {
		request.setAttribute("user", user);
		return "com/xymn/userManage/user-update";
	}

	@RequestMapping(params = "doUpdateTableStatus")
	@ResponseBody
	public AjaxJson doUpdateTableStatus(TSUser user, @RequestParam String tableStatus, HttpServletRequest req) {


			AjaxJson j = new AjaxJson();
			TSUser users=new TSUser();
			if(org.apache.commons.lang3.StringUtils.isNotEmpty(user.getId())){
				 users = systemService.getEntity(TSUser.class, user.getId());
				if(tableStatus.equals("0")){
					users.setTableStatus(1);
				}else{
					users.setTableStatus(0);
				}
			}
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		//设置为普通用户
		String newRoleid="8a8ab0b246dc81120146dc81818b0051,";
		TSUser currentUser = ResourceUtil.getSessionUser();
		String newOrgid=currentUser.getDepartid()+",";
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			users.setBillModel(user.getBillModel());
			users.setQqNumber(user.getQqNumber());
			...
			if (users != null) {
				...
				//密码设置手机后6位
				String password=req.getParameter("mobilePhone").substring(5);
				user.setPassword(PasswordUtil.encrypt(user.getUserName(), oConvertUtils.getString(password), PasswordUtil.getStaticSalt()));
				user.setStatus(Globals.User_Normal);
				user.setDeleteFlag(Globals.Delete_Normal);
				user.setAllowance(0);
				user.setQqNumber(req.getParameter("qqNumber"));
				user.setAllowance(0);//初始化余量
				user.setBillModel(req.getParameter("billModel"));
				user.setAddress(req.getParameter("address"));
				user.setCompany(req.getParameter("company"));
				user.setDevFlag("1");//设置为非开发模式
				//*****************************************初始化用户表************************************************************
				StringBuffer initUserTable=new StringBuffer();
				initUserTable.append(" DROP TABLE IF EXISTS `wl_logistics_order_" + user.getRealName() + "` ; ")
						.append(" CREATE TABLE ")
						.append("  `wl_logistics_order_" + user.getRealName() + "` ")
						.append("  (   `id` varchar(36) NOT NULL, ")
						.append("   `create_name` varchar(50) DEFAULT NULL COMMENT '创建人名称', ")
						.append("   `create_by` varchar(50) DEFAULT NULL COMMENT '创建人登录名称', ")
						.append("   `create_date` datetime DEFAULT NULL COMMENT '创建日期', ")
						.append("   `update_name` varchar(50) DEFAULT NULL COMMENT '更新人名称', ")
						.append("   `update_by` varchar(50) DEFAULT NULL COMMENT '更新人登录名称', ")
						.append("   `update_date` datetime DEFAULT NULL COMMENT '更新日期', ")
						.append("   `sys_org_code` varchar(50) DEFAULT NULL COMMENT '所属部门', ")
						.append("   `sys_company_code` varchar(50) DEFAULT NULL COMMENT '所属公司', ")
						.append("   `bpm_status` varchar(32) DEFAULT '1' COMMENT '流程状态', ")
						.append("   `order_code` varchar(50) DEFAULT NULL COMMENT '订单编号', ")
						.append("   `express_company` varchar(100) DEFAULT NULL COMMENT '快递公司', ")
						.append("   `express_number` varchar(50) DEFAULT NULL COMMENT '快递单号', ")
						.append("   `order_time` datetime DEFAULT NULL COMMENT '订单时间', ")
						.append("   `payment_time` datetime DEFAULT NULL COMMENT '付款时间', ")
						.append("   `delivery_time` datetime DEFAULT NULL COMMENT '发货时间', ")
						.append("   `promise_delivery_time` datetime DEFAULT NULL COMMENT '承诺发货时间', ")
						.append("   `amount` double(10,2) DEFAULT NULL COMMENT '交易金额', ")
						.append("   `consignee` varchar(100) DEFAULT NULL COMMENT '收件人', ")
						.append("   `phone` varchar(32) DEFAULT NULL COMMENT '手机', ")
						.append("   `province` varchar(50) DEFAULT NULL COMMENT '省级', ")
						.append("   `city` varchar(50) DEFAULT NULL COMMENT '市级', ")
						.append("   `region` varchar(50) DEFAULT NULL COMMENT '区级', ")
						.append("   `street` varchar(255) DEFAULT NULL COMMENT '街道', ")
						.append("   `silence_hour` int(3) DEFAULT NULL COMMENT '沉默时间', ")
						.append("   `logistics_tail_time` datetime DEFAULT NULL, ")
						.append("   `logistics_tail_info` varchar(255) DEFAULT NULL COMMENT '物流跟踪信息', ")
						.append("   `user_name` varchar(100) DEFAULT NULL COMMENT '用户名', ")
						.append("   `logistics_tail_status` varchar(10) DEFAULT NULL COMMENT '跟踪状态', ")
						.append("   `store` varchar(100) DEFAULT NULL COMMENT '店铺',")
						.append("   PRIMARY KEY (`id`) ")
						.append("   ) ENGINE=InnoDB DEFAULT CHARSET=utf8; ")
						//
						.append("  DROP TABLE IF EXISTS `wl_logistics_order_product_" + user.getRealName() + "` ; ")
						.append("  CREATE TABLE ")
						.append("  `wl_logistics_order_product_" + user.getRealName() + "` ")
						.append("  (`id` varchar(36) NOT NULL, ")
						.append("  `create_name` varchar(50) DEFAULT NULL COMMENT '创建人名称', ")
						.append("  `create_by` varchar(50) DEFAULT NULL COMMENT '创建人登录名称', ")
						.append("   `create_date` datetime DEFAULT NULL COMMENT '创建日期', ")
						...
						.append("   `sys_company_code` varchar(50) DEFAULT NULL COMMENT '所属公司', ")
						.append("   `bpm_status` varchar(32) DEFAULT '1' COMMENT '流程状态', ")
						.append("   `order_id` varchar(36) DEFAULT NULL COMMENT '主表id', ")
						.append("   `order_code` varchar(50) DEFAULT NULL COMMENT '订单编号',")
						.append("   `product_code` varchar(50) DEFAULT NULL COMMENT '商品编号', ")
						.append("   `sku_code` varchar(50) DEFAULT NULL COMMENT 'SKU编号', ")
						.append("   `outside_code` varchar(50) DEFAULT NULL COMMENT '外部编号', ")
						.append("   `product_name` varchar(100) DEFAULT NULL COMMENT '商品名称', ")
						.append("   `product_specification` varchar(50) DEFAULT NULL COMMENT '商品规格', ")
						.append("   `product_unit_price` double DEFAULT NULL COMMENT '商品单价', ")
						.append("   `product_num` int(11) DEFAULT NULL COMMENT '商品数量', ")
						.append("    PRIMARY KEY (`id`) ")
						.append("    ) ENGINE=InnoDB DEFAULT CHARSET=utf8; ")
						//
						.append("  DROP TABLE IF EXISTS `wl_kuaidiniao_call_back_" + user.getRealName() + "` ;  ")
						.append("  CREATE TABLE `wl_kuaaidiniao_call_back_" + user.getRealName() + "` ")
						.append("  (`id` varchar(36) NOT NULL DEFAULT '' COMMENT 'id', ")
						.append("  `push_time` varchar(32) DEFAULT NULL COMMENT '更新时间', ")
						.append("  `ebusiness_id` varchar(50) DEFAULT NULL COMMENT '快递鸟用户电商ID', ")
						.append("  `order_code` varchar(50) DEFAULT NULL COMMENT '订单编号', ")
						.append("  `shipper_code` varchar(20) DEFAULT NULL COMMENT '快递编码', ")
						.append("  `logistic_code` varchar(50) DEFAULT NULL, ")
						.append("  `success` varchar(10) DEFAULT NULL COMMENT '是否成功', ")
						.append("  `reason` varchar(150) DEFAULT NULL COMMENT '失败原因', ")
						.append("  `state` varchar(10) DEFAULT NULL COMMENT '0-无轨迹，1-已揽收，2-在途中，3-签收,4-问题件', ")
						.append("  `call_back` varchar(50) DEFAULT NULL, ")
						.append("  `traces` text COMMENT '物流信息', ")
						.append("  `refresh_time` datetime DEFAULT NULL COMMENT '回调时间', ")
						.append("   PRIMARY KEY (`id`) ")
						.append("   ) ENGINE=InnoDB DEFAULT CHARSET=utf8; ")
						//
						.append("  DROP TABLE IF EXISTS `wl_send_back_" + user.getRealName() + "` ;  ")
						.append("  CREATE TABLE `wl_send_back_admin_" + user.getRealName() + "`")
						.append("  ( `id` varchar(36) NOT NULL DEFAULT '' COMMENT 'id',")
						.append("   `logistic_code` varchar(50) DEFAULT NULL COMMENT '快递单号', ")
						.append("   `shipper_code` varchar(30) DEFAULT NULL COMMENT '快递编码', ")
						.append("   `order_code` varchar(50) DEFAULT NULL COMMENT '订单编号', ")
						.append("   `update_time` varchar(36) DEFAULT NULL COMMENT '更新时间', ")
						.append("   `ebusiness_id` varchar(36) DEFAULT NULL COMMENT '账户id', ")
						.append("   `success` varchar(10) DEFAULT NULL COMMENT '状态', ")
						.append("   `reason` varchar(150) DEFAULT NULL COMMENT '发送失败原因', ")
						.append("   `call_back` varchar(50) DEFAULT NULL COMMENT '传送指定参数 特指用户id', ")
						.append("   PRIMARY KEY (`id`) ")
						.append("  ) ENGINE=InnoDB DEFAULT CHARSET=utf8; ");
				//*****************************************初始化用户表************************************************************
				 systemService.executeSql(initUserTable.toString());
				 user.setTableStatus(0);//设置停用
				 user.setTableName("database_"+user.getUserName());
				this.userService.saveOrUpdate(user, newOrgid.split(","), newRoleid.split(","));
				//初始化用户表
	...
	public ModelAndView roles(HttpServletRequest request) {
		...
		ModelAndView mv = new ModelAndView("com/xymn/userManage/users");
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
        ...
        return new ModelAndView("com/xymn/userManage/user");
	}
    /**
     * easyuiAJAX请求数据： 跳转用户充值
     *
     * @param request
     * @param response
     * @param dataGrid
     * @param user
     */
    @RequestMapping(params = "goRechargeOrRenewals")
    public ModelAndView goRechargeOrRenewals(TSUser user, HttpServletRequest req) {
        List<String> orgIdList = new ArrayList<String>();
        TSDepart tsDepart = new TSDepart();
        if (StringUtil.isNotEmpty(user.getId())) {
            user = systemService.getEntity(TSUser.class, user.getId());
        ...
        return new ModelAndView("com/xymn/userManage/rechargeOrRenewals");
    }
	/**
	 * 用户充值续费录入
	 *
	 ...
	 * @return
	 */
	@RequestMapping(params = "saveRechargeOrRenewals")
	@ResponseBody
	public AjaxJson saveRechargeOrRenewals(HttpServletRequest req, TSUser user){
		String message = null;
		AjaxJson j = new AjaxJson();
		Short logType=Globals.Log_Type_UPDATE;
		...
		String ralName=req.getParameter("realName");//姓名
		String mobile=req.getParameter("mobilePhone");//手机
		String company=req.getParameter("company");//公司名称
		String billModel=req.getParameter("billModel");//计费模式
		SalesanalysisInfoEntity salesanalysisInfoEntity=new SalesanalysisInfoEntity();
		try{
			if(StringUtil.isNotEmpty(billModel)){
				if(billModel.equals("0")){
					//1.按单（充值）
					String rechargeAmount=req.getParameter("rechargeAmount");//充值金额
					String unitPrice=req.getParameter("unitPrice");//单价
					String rechargeQuantity=req.getParameter("rechargeQuantity");//充值数量
					String rechargeAfterQuantity=req.getParameter("rechargeAfterQuantity");//充值后数量
					salesanalysisInfoEntity.setCreateDate(new Date());//交易时间
					salesanalysisInfoEntity.setCreateBy(ResourceUtil.getSessionUser().getCreateBy());//操作员
					salesanalysisInfoEntity.setRechargeType(Integer.valueOf(billModel));//类型
					salesanalysisInfoEntity.setTradingAmount(Double.valueOf(rechargeAmount));//交易金额
					salesanalysisInfoEntity.setUserName(userName);//用户名
					salesanalysisInfoEntity.setRealName(ralName);//姓名
					salesanalysisInfoEntity.setCompany(company);//公司名称
					salesanalysisInfoEntity.setMobilePhone(mobile);//手机
					salesanalysisInfoEntity.setPayUnitPrice(Double.valueOf(unitPrice));//单价
					salesanalysisInfoEntity.setPayNumber(Integer.valueOf(rechargeQuantity));//充值数量
					if (StringUtil.isNotEmpty(user.getUserName())) {
						List<TSUser> users = systemService.findByProperty(TSUser.class,"userName", user.getUserName());
						if(users!=null||users.size()>0){
							users.get(0).setAllowance(Integer.valueOf(Integer.valueOf(rechargeAfterQuantity)));
							users.get(0).setTableStatus(1);//开启表状态
							systemService.saveOrUpdate(users.get(0));
							message = "用户: " + users.get(0).getUserName() + "充值成功";
						}
					}
				}else{
					//2.按天（续费）
					//String endTime=req.getParameter("endTime");//到期时间
					String rachargeMoney=req.getParameter("rachargeMoney");//续费金额
					String rechargeDay=req.getParameter("rechargeDay");//续费天数
					String rechargeEndTime=req.getParameter("rechargeEndTime");//续费到期时间
					salesanalysisInfoEntity.setCreateDate(new Date());//交易时间
					salesanalysisInfoEntity.setCreateBy(ResourceUtil.getSessionUser().getCreateBy());//操作员
					salesanalysisInfoEntity.setRechargeType(Integer.valueOf(billModel));//类型
					salesanalysisInfoEntity.setTradingAmount(Double.valueOf(rachargeMoney));//交易金额
					salesanalysisInfoEntity.setUserName(userName);//用户名
					salesanalysisInfoEntity.setRealName(ralName);//姓名
					salesanalysisInfoEntity.setCompany(company);//公司名称
					salesanalysisInfoEntity.setMobilePhone(mobile);//手机
					salesanalysisInfoEntity.setRechargeDay(Integer.valueOf(rechargeDay));
					if (StringUtil.isNotEmpty(user.getUserName())) {
						List<TSUser> users = systemService.findByProperty(TSUser.class,"userName", user.getUserName());
						if(users!=null||users.size()>0){
							users.get(0).setEndTime(new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse(rechargeEndTime));
							users.get(0).setTableStatus(1);//开启表状态
							systemService.saveOrUpdate(users.get(0));
							message = "用户: " + users.get(0).getUserName() + "续费成功";
						}
					}
				}

			}
			systemService.save(salesanalysisInfoEntity);
		}catch(Exception e){
			e.printStackTrace();
		}
		systemService.addLog(message, logType, Globals.Log_Leavel_INFO);
		j.setMsg(message);
		logger.info("["+IpUtil.getIpAddr(req)+"][用户充值/续费成功]"+message);
		return j;
	...
	public ModelAndView addorupdateInterfaceUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/xymn/userManage/interfaceUser");
	}
...
//                saveUserOrgList(req, user);
	...
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		...
		return new ModelAndView("com/xymn/userManage/userOrgSelect");
    }
	...
	public String choose(HttpServletRequest request) {
		...
		return "com/xymn/userManage/checkuser";
	}
	...
	public String roleDepart(HttpServletRequest request) {
		List<TSRole> roles = systemService.loadAll(TSRole.class);
		request.setAttribute("roleList", roles);
		return "com/xymn/userManage/roledepart";
	}
	...
	public ModelAndView chooseDepart(HttpServletRequest request) {
		...
		if (nodeid.equals("role")) {
			modelAndView = new ModelAndView("com/xymn/userManage/users");
		} else {
			modelAndView = new ModelAndView("com/xymn/userManage/departList");
		}
		return modelAndView;
	}
	...
	public ModelAndView addsign(HttpServletRequest request) {
		...
		return new ModelAndView("com/xymn/userManage/usersign");
	}
...
//		request.setAttribute("cssTheme", cssTheme);
		return "com/xymn/userManage/changestyle";
	}
	/**
	* @Title: saveStyle
	* @Description: 修改首页样式
...
//			}
	...
	public String userSelect() {
		return "com/xymn/userManage/userSelect";
	}
	...
	public ModelAndView addorupdateMyOrgUser(TSUser user, HttpServletRequest req) {
		...
		req.setAttribute("tsDepart", tsDepart);
        return new ModelAndView("com/xymn/userManage/myOrgUser");
	}
    /**
     * 公司名称自动补全
     * @return
     ...
     * @throws IOException
     */
    @RequestMapping(params = "companyNameAutoComplete" )
    @ResponseBody
    public void scNameAutoComplete(String state, HttpServletResponse response) throws IOException{
        List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
        StringBuffer sql = new StringBuffer()
                .append("select t.company AS company")
                .append("FROM ")
                .append("t_s_user AS t ");
